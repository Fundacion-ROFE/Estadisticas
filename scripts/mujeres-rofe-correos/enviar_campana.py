...
# -*- coding: utf-8 -*-
"""
Envío de correos parametrizados para Mujeres ROFÉ (v2 — seguro + reutilizable)
...
SEGURIDAD:
  - Contraseña SIEMPRE en variable de entorno SMTP_PASSWORD (no en código)
...
TOOLS_DATA = PROYECTO_ROOT / "tools" / "mujeres-rofe-correos" / "data"
TOOLS_DATA.mkdir(parents=True, exist_ok=True)


def cargar_env_local():
    """Carga .env.local (raíz del repo) al entorno SIN sobreescribir lo ya definido.
    Se llama ANTES de construir CONFIG_SMTP para que SMTP_*/SUPABASE_* estén disponibles.
    No pisa lo que ya puso run_piloto.py (getpass) ni las variables del shell."""
...
CONFIG_SMTP = {
    ...
    "SMTP_PORT": 587,
    "SMTP_USER": os.environ.get("SMTP_USER", "mujeres.rofe@tocaunavida.org"),
    "SMTP_PASSWORD": os.environ.get("SMTP_PASSWORD"),  # ← OBLIGATORIO EN VARIABLE
    "FROM_NAME": "Equipo Mujeres ROFÉ",
    "REPLY_TO": os.environ.get("SMTP_REPLY_TO", "mujeres.rofe@tocaunavida.org"),
    # Imágenes inline
    "IMG_BANNER": BASE / "img" / "banner.png",
    "IMG_FIRMA": BASE / "img" / "firma.png",
    # Plantilla base
    "PLANTILLA_TEMPLATE": BASE / "templates" / "email_v2_template.html",
}
...
def registrar_campana_supabase(campana_id, enviados, fallidos, programa):
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "panel-datos-etl/1.0",  # NO Mozilla (Supabase bloquea secrets "de navegador")
...
def verificar_config_smtp():
    ...
    if not CONFIG_SMTP["SMTP_PASSWORD"]:
        raise ValueError(
            "ERROR CRITICO: falta variable de entorno SMTP_PASSWORD\n"
            "Define en PowerShell:\n"
            '  $env:SMTP_PASSWORD = "xxxx xxxx xxxx xxxx"  # contraseña app 16 dígitos\n'
        )
    if not CONFIG_SMTP["SMTP_USER"]:
        raise ValueError("ERROR: falta SMTP_USER en variables de entorno")
    print(f"[OK] Config SMTP validada: {CONFIG_SMTP['SMTP_USER']} en {CONFIG_SMTP['SMTP_HOST']}")

def verificar_imagenes():
    """Verifica que existan las imagenes inline."""
    faltan = []
    for clave, ruta in [("banner", CONFIG_SMTP["IMG_BANNER"]), ("firma", CONFIG_SMTP["IMG_FIRMA"])]:
        if not ruta.exists():
            faltan.append(f"{clave}: {ruta}")
    if faltan:
        raise FileNotFoundError(f"Faltan imagenes:\n  " + "\n  ".join(faltan))
    print(f"[OK] Imagenes verificadas ({CONFIG_SMTP['IMG_BANNER']}, {CONFIG_SMTP['IMG_FIRMA']})")
...
def nuevos_cids():
    """Genera IDs únicos para imagenes inline."""
    return {
        "banner": make_msgid(domain="mujeresrofe"),
        "firma": make_msgid(domain="mujeresrofe"),
    }

def construir_mensaje(dest_nombre, dest_correo, html, cids, vars_campana, adjunto=None):
    """Construye EmailMessage con HTML + imagenes inline.
    adjunto: tupla opcional (ruta_archivo, nombre_archivo) para adjuntar un PDF
    propio de este destinatario (ej. certificados personalizados)."""
    msg = EmailMessage()
    msg["Subject"] = vars_campana.get("ASUNTO", "Mujeres ROFÉ")
    ...
    texto = (
        ...
        f"Para más información: {vars_campana.get('URL_CTA', 'https://tocaunavida.org')}\n\n"
        f"Equipo Mujeres ROFÉ"
    )
    msg.set_content(texto)

    # HTML con variables interpoladas
    html_final = Template(html).safe_substitute(**vars_campana)
    # Reemplaza cid por IDs generados
    html_final = html_final.replace("cid:banner", "cid:" + cids["banner"].strip("<>"))
    html_final = html_final.replace("cid:firma", "cid:" + cids["firma"].strip("<>"))
    msg.add_alternative(html_final, subtype="html")

    # Adjunta imagenes inline (solo si la plantilla realmente las referencia —
    # una plantilla sin cid:banner, por ejemplo, no debe cargar banner.png de todos modos)
    payload = msg.get_payload()[1]  # parte HTML
    for clave, ruta in [("banner", CONFIG_SMTP["IMG_BANNER"]), ("firma", CONFIG_SMTP["IMG_FIRMA"])]:
        if ("cid:" + cids[clave].strip("<>")) in html_final and ruta.exists():
            data = ruta.read_bytes()
            subtype = ruta.suffix.lstrip(".").lower() or "png"
            if subtype == "jpg":
                subtype = "jpeg"
            payload.add_related(data, maintype="image", subtype=subtype, cid=cids[clave])

    if adjunto:
        ruta_adjunto, nombre_adjunto = adjunto
...
def accion_preview(plantilla, campana, archivo_salida="preview.html"):
    ...
    vars_demo["NOMBRE"] = "María (DEMO)"
    vars_demo["ASUNTO"] = vars_demo.get("ASUNTO", "[Demo]")

    html_final = Template(plantilla).safe_substitute(**vars_demo)
    # Refleja el banner/firma REALES de esta campaña (puede venir sobreescrito por
    # IMG_BANNER/IMG_FIRMA del JSON) — no siempre son los genéricos img/banner.png.
    banner_rel = CONFIG_SMTP["IMG_BANNER"].relative_to(BASE).as_posix()
    firma_rel = CONFIG_SMTP["IMG_FIRMA"].relative_to(BASE).as_posix()
    html_final = html_final.replace("cid:banner", banner_rel)
    html_final = html_final.replace("cid:firma", firma_rel)

    Path(archivo_salida).write_text(html_final, encoding="utf-8")
    print(f"[OK] Preview guardado: {archivo_salida}")
    print(f"     Abre en navegador para revisar HTML, colores, imagenes, responsivo.")
...
def accion_piloto(plantilla, campana, correo_piloto, archivo_registro):
    ...
    try:
        cids = nuevos_cids()
        msg = construir_mensaje("Piloto Mujeres ROFÉ", correo_piloto, plantilla, cids, vars_piloto)
        s.send_message(msg)
...
def main():
    parser = argparse.ArgumentParser(
        description="Envío parametrizado para Mujeres ROFÉ (v2 segura + reutilizable)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
EJEMPLOS:
  python enviar_campana.py --preview campana_bienvenida_2023.json
  python enviar_campana.py --piloto campana_bienvenida_2023.json samueldavidvida@gmail.com
  python enviar_campana.py --enviar campana_bienvenida_2023.json
        """)
    ...
    # Imágenes propias de la campaña (opcional, ej. banner de un evento puntual) —
    # si el JSON no las declara, se usan las genéricas img/banner.png e img/firma.png.
