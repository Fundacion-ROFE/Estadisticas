...
- `anomalias.inhabilitados_sin_retiro` (5 casos al 2026-07-07 → 2 al 2026-07-09 → **0 tras normalizar**): inhabilitados que no están en el reporte de cancelados. Los 2 últimos (Samuel Murcia `1034662377`, Vicenzo Vecchio `58464721`) resultaron ser matriculados 2026 de JC que **ya habían completado el programa** (100% en 6/7 cursos) y Q10 archivó sin cancelación formal — no eran desertores reales pero inflaban retirados a 82. Se **normalizaron descartándolos** vía `tools/exclusiones_prueba.json` (motivo `normalizado`), dejando retirados en **80** y la identidad `cohorte = activos + retirados` exacta (855 = 775 + 80). Si esta anomalía vuelve a crecer, investigar caso por caso antes de excluir: puede ser un desertor real aún sin cancelación registrada.
