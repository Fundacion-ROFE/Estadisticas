...
| Panel de Control JC/MR (herramienta nueva, PII, fuentes togglables) | [[panel-control-jc-mr]] | **Fases 1-3 hechas (2026-07-30).** Surgió de una auditoría de geografía JC que encontró 43 matriculados del área metropolitana de Medellín (Envigado/Sabaneta/Itagüí) con 0% de cobertura en la BD de Seguimiento (ver [[bd-seguimiento-monitorias]]). Reemplaza a `panel_riesgo_gui.py`; reusa `v_gui_personas` y `v_persona_360` sin duplicar lógica de cruce. Verificado sin traceback, no verificado visualmente (sin herramienta de captura en este entorno). Bloqueado por: revisión visual de Samuel antes de Fase 4 (ficha 360) |
| Rediseño del panel de datos público (Netlify, desde cero) | [[plan-rediseno-panel-datos-2026-07-30]] | **Solo especificación, sin plan de fases todavía (2026-07-30) — a pedido explícito de Samuel.** Diagnóstico: sin filtro real de "habilitados/todos" (solo KPIs pasivos), lógica de aprobación duplicada entre tabs Resumen/Cursos, Demografía es 2 tabs distintos disfrazados de uno, Asistencia Zoom no existe en el panel público. Bloqueado por: 4 preguntas abiertas en el documento (stack, si entra Asistencia, alcance del filtro de estado, prioridad frente a `panel-control-jc-mr`) |
...
| Gobernanza de contexto IA por usuario | [[gobernanza-contexto-ia]] | **Roles activados (2026-08-03):** Lina, Rocío y Cristian ya tienen su `usuarios-ia/<nombre>/CLAUDE.md` completo (URL de su propia carpeta, conexión Supabase de solo lectura, skills según su rol, y el bloque fijo "Límites de autonomía": si algo no encaja, la instancia dice explícitamente que necesita luz verde de Samuel, nunca inventa un dato ni busca un atajo). Astrid y Sandra quedan bloqueadas — sin skills ni acceso — hasta levantar su propia entrevista de necesidades. **Hallazgo importante:** el repo donde vive esto (`Fundacion-ROFE/Estadisticas`) es público; el diseño siempre asumió un repo privado aparte, así que activar `logs/` de sesión reales queda bloqueado hasta migrar. **Corrección técnica:** el disparador de push ya no es un Schedule n8n (no puede ver las máquinas de otras personas) sino un hook `Stop` local de Claude Code por persona; `commit_y_push.py` ahora soporta `--usuario <nombre>` para acotar el push a una sola carpeta. Bloqueado por: crear el repo privado real en GitHub (fuera de sesiones de código), configurar el hook en cada máquina. |
