...
| Gobernanza de contexto IA por usuario | [[gobernanza-contexto-ia]] | **Scaffolding inicial (2026-07-27)** + **confirmado en entrevistas P0 (2026-07-28):** estructura `usuarios-ia/<nombre>/` (CLAUDE.md + skills/ + logs/) y scripts `scan_pii.py`/`commit_y_push.py` creados (reutilizan la detección del pseudonimizador; bloquean el push si hay PII sin pseudonimizar). Confirmado con el equipo: revisión semanal del repo + alerta antes de acción dudosa, como prerrequisito de las instancias individuales de Claude Workspace pedidas por Lina/Rocío/Cristian (Astrid y Sandra aún sin levantar del todo — ver [[gobernanza-contexto-ia]]). Bloqueado por: crear el repo privado real en GitHub (fuera de esta sesión), dar de alta usuario piloto, conectar a Schedule n8n + alerta Telegram. |
