...
| Panel de Control JC/MR (herramienta nueva, PII, fuentes togglables) | [[panel-control-jc-mr]] | **Solo documentación, sin ejecutar (2026-07-30) — a pedido explícito de Samuel.** Surgió de una auditoría de geografía JC que encontró 43 matriculados del área metropolitana de Medellín (Envigado/Sabaneta/Itagüí) con 0% de cobertura en la BD de Seguimiento (ver [[bd-seguimiento-monitorias]]). Reemplaza a `panel_riesgo_gui.py` (herramienta nueva, no evolución); reusa `v_gui_personas` y `v_persona_360` sin duplicar lógica de cruce. Bloqueado por: nada técnico — el plan está listo, falta empezar la Fase 1 |
...
| Validación de identidad del asistente Zoom | [[zoom-asistencia]] | **AUTOMATIZADO en producción (2026-07-30):** `validar_asistencia.py` encadenado antes del sync en `asistencia-zoom-diario` (n8n) — pestaña `ASISTENCIA-VALIDADA`. Primera corrida real: 1249 filas, 84,4% correo exacto (excl. no-clase/staff), 102 a revisión manual. Verificado end-to-end en n8n (ejecución 1282, cadena completa OK, `v_frescura` al día). **🔴 Hallazgo para el equipo:** la columna `Identificacion` viene 0% llena — el formulario de Zoom no la captura, el camino "ID corrige correo" nunca se usa. |
